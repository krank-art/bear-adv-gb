...
entDefs={
 ...
 coin={
  ...
  sld=false, -- is solid; when true, gets treated as solid entity, see sldEnts
  bhv={"coin"}, -- behaviour
...
function TIC()
 ...
 print(tableToString(sldCols),100,2,15)
...
function drwEnts()
 -- first, solid entities
 for i,e in pairs(sldEnts) do drwEnt(e) end
 -- then normal entities
...
function loadEnts()
 sldEntCnt = loadEntGrp(sldEnts)
 entCnt = loadEntGrp(ents)
end

function loadEntGrp(entGrp)
 local entGrpCnt=#entGrp+1
 for i,e in ipairs(entGrp) do
...
function loadMap()
 ...
 local function addEnt(eid,i,j)
  local def=entDefs[eid]
  local isSldEnt=def.sld and def.sld==true
  local entGrp=tern(isSldEnt, sldEnts, ents)
  local entGrpCnt=tern(isSldEnt, sldEntCnt, entCnt)
  table.insert(entGrp, {id=eid,x=i*8,y=j*8})
  entGrpCnt=#entGrp+1
 ...
 local onTile = {
  ...
  [206] = function(tid,i,j) addEnt("anblo",i,j) end,
 }
...
--  animation state, x, y, [flp=0], [ck=1])
--  flp: flip animation; ck: color key
function plyAni(cani,tml,state,x,y,flp,colKey)
 flp,colKey=flp or 0,colKey or 1
 ...
 local t,d,fx,fy,w,h,fl,ck=f[1],f[2],f[3],f[4],f[5],f[6],f[7] or 0,f[8] or colKey
 local flpSpr=(flp+fl)%2 -- (flip animation + flip frame) % 2
 spr(t,x+fx,y+fy,ck,1,flpSpr,0,w,h)
...
-- tests if player collides with solid entities
function updSldCol()
 local px,py,pw,ph,pcx,pcy=plr.x,plr.y,plr.w,plr.h,plr.cx,plr.cy
 local m=32 -- margin
...
--  returns the max possible velocity to 
--  either hit nothing or reach a solid entity
function sldSwpEntPlr(axis)
 ...
 if axis==0 and vx>0 then
  ...
  v=wx -- initially max value in dir
 ...
 for _,eid in pairs(sldCols) do -- collision id, entity
...
-- 104:55555555500455551ef255558aeb4555caef4555c20f0000c21ffff3c31bfaa2
-- 105:555555555510045555cff35551befb4551b00b4551f41a4551a00a455cffff35
-- 106:500055551fff4555caff3555c20c2555c35c2555005825555518200455cffff3
...
-- 120:c31bbaa2c30b0fc3cfa21a828fa210c31ef45504500555555555555555555555
-- 121:58affa355ce3ca355ce3ca355ce3ca355cefba355caaaa255100004555555555
-- 122:558affa355ceffa355ceffa355ceffa355cefba355caaaa25510000455555555
...
-- 199:cff0cff0fff3fff33c033f33bc23bc23bc23bc23fff3fff3cff0cff000000000
...
-- 215:0f300f308fb0c0c0cee0c0c0cbf0fff3cee0f3f38fb0f3f3cff0fff300000000
...
-- 096:55504550551a251f551a20cf551affcf551effff551fffff551ffff3551ffff3
-- 097:55555555455555103555550f355555ca355555ca355555cfb45555cff45555cf
-- 098:5555555555105555458a4555308a45553ffb2555ffff2555fcfc3555fcfcf455
-- 112:551ffff3551ffffa551fffff510effff5c3befff5cffffff5cffffff5cffffff
...
-- 128:510ff8ff551f3fef551f3fcf555cf0ff555cffff551ff300551f345555500555
-- 129:f3555cfcf345500fff35551fff35551f0c35550f514555cf555555cf55555510
...
-- 004:
-- 005:
...
-- 012:
-- 013:
